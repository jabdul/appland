define([
  'main',
  'lib/requirejs/domReady!',
  'lib/requirejs/i18n!nls/conf',
  'lib/requirejs/i18n!module-demo-web-app/nls/conf',
  'core/connect',
  'log4javascript'],
function (App, Doc, AppConfig, ModuleConfig, Connect, Log4j) {
  App.setConfig({
    'projectName': ModuleConfig['module-demo-web-app'].projectName
  });

  // Setup logger
  // The getLogger string argument must be prefixed with Apl.Modules.
  ModuleConfig['module-demo-web-app'].Log =
    Log4j.getLogger( 'Apl.Modules.DemoWebApp' );
  // Enable / disable logging
  App.setLogging({
      'setEnabled': AppConfig.App.Logging.setEnabled
  });

	App.setModuleConfig({
		'module-demo-web-app': ModuleConfig['module-demo-web-app']
	});

	var connect = new Connect(AppConfig.Server);
	App.setConnection(connect);
	return App;
});